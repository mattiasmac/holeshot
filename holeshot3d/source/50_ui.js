...
const UI = (function () {
  ...
  function renderMenu() {
    ...
    const gs = (typeof GL !== 'undefined') ? GL.status() : { mode: 'classic' };
    $('#menuCar').innerHTML = `<div class="swatch" style="background:${car.color};border:1px solid #444"></div><div class="grow"><div class="name">${car.maker} ${car.name}</div><div class="spec">${car.cls} · ${Math.round(Game.specFor(car.id).hp)} hp · dyno ${fmt(d.et)} @ ${d.mph.toFixed(0)}</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px"><div class="points">${num(P.points)} pts</div><span class="chip ${gs.mode === '3d' ? 'on' : ''}" title="${gs.why || ''}">${gs.mode === '3d' ? '3D' : 'CLASSIC 2D'}</span></div>`;
  ...
  function renderSettings() {
    ...
    seg('#segGfx', s.gfx || '3d', v => { s.gfx = v; Game.save(); setTimeout(() => location.reload(), 250); });
    { const gs = (typeof GL !== 'undefined') ? GL.status() : { mode: 'classic', why: '' }; $('#gfxStatus').textContent = `Now: ${gs.mode === '3d' ? '3D' : 'Classic 2D'} — ${gs.why}`; }
  ...
  function init() {
    ...
    R.attach($('#stage'), $('#gl'), (Game.P.settings.gfx || '3d') !== 'classic'); R.clear();
